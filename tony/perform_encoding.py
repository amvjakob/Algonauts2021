...
def get_model(n_components=100):
    model = keras.Sequential()
    model.add(keras.Input(shape=(n_components,)))
...
def predict_fmri_model(model, train_activations, test_activations, train_fmri, use_gpu=False):
    ...
    ntrain, nvoxels = train_fmri.shape
    ntest, ncomponents = test_activations.shape

    ytest = np.zeros((ntest, nvoxels))

    # fit model for each voxel independently and predict
    for voxel in range(nvoxels):
        model.fit(train_activations,
                  train_fmri[:, voxel:voxel+1], batch_size=32, epochs=10)
        ytest[:, voxel:voxel+1] = model.predict(test_activations)
...
def main():
    ...
    iter = 0
    n_components = 100
    model = get_model(n_components=n_components)
    while iter < num_voxels-batch_size:
        model, pred = predict_fmri_model(model,train_activations,test_activations,fmri_train[:,iter:iter+batch_size], use_gpu = use_gpu)
        pred_fmri[:, iter:iter+batch_size] = pred
